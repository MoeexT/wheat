...
\subsection{\hei\xiaosan\textbf{本文卷积网络结构}}
  ...
  第一层接受的输入\zs 层大小为$100\times100\times3$，所以输出的尺寸为$100+3-1=98$，深度为6。
...
\subsection{\hei\xiaosan\textbf{LeNet-5和AlexNet结构简介}}
  ...
  \end{figure}
  第一层输入的是原始像素，LeNet-5模型接受的输入层\zs 大小为$32\times32\times1$。
  第一个卷积层过滤器\zs 尺寸为$5\times5$，深度为6，不使用全零填充，步长为1，
  ...
  第二层池化层的输入\zs 是$28\times28\times6$的节点矩阵。该层的过滤器大小为$2\times2$，步长也为2。
  所以本层输出\zs 大小为$14\times14\times6$。第三层是输入大小为$14\times14\times6$的卷积层，卷积核大小为
