...
def read_and_show(filename):
    ...
    filename_queue = tf.compat.v1.train.string_input_producer([filename])  # 读入流中
...
def main(times):
    ...
    y_train = tf.python.keras.utils.to_categorical(y_train, num_classes=NUM_CLASSES)
    y_test = tf.python.keras.utils.to_categorical(y_test, num_classes=NUM_CLASSES)
